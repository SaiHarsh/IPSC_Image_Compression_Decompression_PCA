...
#define MAX_ITERATION_COUNT 3000   // Maximum number of iterations
	...
	for (k = ncols - 1; k >= 0; k--) {
		...
		while(1) {
			...
			if (m == k ) {
				...
				if ( iteration_count >= MAX_ITERATION_COUNT*10000 ) return -1;
				iteration_count++;
				//printf("%lld\n",iteration_count );
				x = diagonal[m];
				y = diagonal[k-1];
		...
		double* singular_values, double* V, double* dummy_array)
{
	...
	if (Givens_Reduction_to_Diagonal_Form( nrows, ncols, U, V,
		...
		printf("Can't generated Givens_Reduction_to_Diagonal_Form\n");
		return -1;
	}
